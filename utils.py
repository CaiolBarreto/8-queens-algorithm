...
def print_chromosome(dna, max_fitness):
    integer_chromosome_array = [binary_to_integer(gene) for gene in dna.chromosome]
    print(
        "Chromosome = {},  Fitness = {}".format(str(integer_chromosome_array), dna.fitness(max_fitness))
    )
...
def print_generation(generation, max_fitness, population):
    print("=== Generation {} ===".format(generation))
    print(
        "Maximum Fitness = {}".format(
            max([DNA.fitness(max_fitness) for DNA in population])
        )
    )
